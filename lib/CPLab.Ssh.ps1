...
# All JSON goes to files; only CCES_ lines are meant for the log.
...
say "set accepted (last-modifier: $(grep -o '"last-modifier" : "[^"]*"' /tmp/cces_set.json | head -1 | cut -d'"' -f4))"
...
'@
    ...
    $r = Invoke-CPBash -Session $Session -Command 'bash /home/admin/cces_endpoint_cfg.sh 2>&1' -TimeoutSec (60 * ($TaskTimeoutMin + 10)) -Quiet

    foreach ($l in ($r.Output -split "`n")) {
        if ($l -match '^CCES_INFO:(.*)$') { Write-CPLog "    $($Matches[1])" INFO }
    }
    ...
    if ($r.Output -match 'after\s+endpoint-policy = true') {
